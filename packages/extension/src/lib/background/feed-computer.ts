 ...
 */
const QUIET_POSTER_THRESHOLD = 1;
	...
	private async identifyQuietPosters(
		...
		for (const follow of follows) {
			try {
				...
				const postsPerDay = (recentPosts.length / QUIET_POSTER_LOOKBACK_DAYS) * 1;
		...
		return quietPosters.toSorted((a, b) => (b.lastPostAt ?? 0) - (a.lastPostAt ?? 0));
	}
